...
- **Headline gap visible at n=3**: criterion pass rate 61 to 69 %, full-prompt pass rate **0 %**. The constraint-satisfaction gap the runbook predicted is already detectable on three prompts.
...
Deliverable matches `meta/RESULTS_SCHEMA.md` at `schema_version 1.0`.
Intermediate + sidecar files this run also wrote:
...
| `outputs/experiments/E01-smoke-3p.json` | the deliverable: dashboard binds to this file for this experiment. |
| `outputs/experiments/index.json` | dashboard dropdown source; this run added the `E01-smoke-3p` entry. |
| `outputs/results.json` | canonical latest-run copy of the deliverable (equal to the E01-smoke-3p file until the next run). |
| `outputs/run_manifest.json` | mirror of the meta block plus the `judge_agreement` block (the latter merged by `validate --mode score`). |
