...
1. **The Implicit weakness decomposes into retention, not comprehension.** E05's hardest category (Implicit, 63 to 68% pass) is dominated by `constraint_dropped` (27) and `degenerate_output` (15, all one qwen-9b response), not `constraint_misread` (7). Models mostly *do* infer the implicit constraints; they fail to carry them into the final answer.
