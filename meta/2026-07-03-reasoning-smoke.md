...
openrouter:qwen/qwen3.5-9b (RuntimeError)` at 3 to 20-minute intervals: each
...
2. **Default routing drew a ~20 to 25 tok/s provider** (~9 500 CoT chars in 120 s); a full 32k-token generation at that rate is ~25 min, matching the observed attempt durations exactly. A second probe drew a ~110 tok/s provider (20 002 chars in 45 s): the provider lottery spans ~10×, consistent with the orphan that burned 32k in 3.2 min (~164 tok/s).
...
truncations. Thinking traces ran 26 894 to 63 408 chars (~7k to 16k tokens); visible
answers 427 to 6 292 chars. One transient empty-body retry (qwen-35b CIF-001)
...
Opus view (Fable tracks within 0 to 2 criteria on every cell):
...
$1 to $3: the cost of the config search, not the experiment.
...
- **Prior**: E04 smoke: +14 to +29 pp, Multistep (CIF-003) gains largest for small/medium models. Expect shrinkage at n=75 but same sign everywhere; 397b full-prompt pass rate 5 to 15% (vs 0% baseline).
...
- **Cost / wall-clock**: ~$120 to $240, ~26 hr sequential / ~8 hr with 4-way generate parallelism.
...
- **Cost / wall-clock**: one E03-sized run: ~30 min, ~$5 to $8.
...
- **Cost / wall-clock**: one E04-sized run (~$6 to $12, ~1.5 hr).
...
1. **Parallelize `generate.run()`** (small thread pool over candidates, ~1 sitting). **Why:** reasoning made generation the bottleneck (~18 hr sequential for 75p); 3 to 4× parallelism makes Experiment 1 feasible in one overnight run. [carried from E01 list, now blocking]
