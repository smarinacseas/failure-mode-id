...
- **Implicit instructions are the hardest category** (63 to 68% criterion pass, Opus), a category E01-E04 never saw (the first-3 sample was Negative/Negative/Multistep). Conditional is the easiest (88.7 to 90.9%). This is exactly the kind of blind spot the stratified sampler was built to expose.
- **The judges agree**: 96.8% criterion-level verdict agreement (excluding the refused prompt), Fable ~2 to 3 pp stricter than Opus everywhere, aggregate gap ≤ 2.9 pp per model.
...
Reasoning traces ran 8.6k to 89k chars (median ~27k to 40k by model); visible
answers 242 to 87,676 chars. qwen-397b thinks the least (median 27k chars) and
...
doesn't rescue it. The three passable prompts are mid-length (10 to 18 criteria),
...
### Per-prompt (criteria passed, Opus view; Fable tracks within 0 to 2 criteria everywhere except CIF-006)
...
- Fable is systematically ~2 to 3 pp stricter than Opus on identical responses. Direction is consistent across all models, so model *rankings* are judge-robust, absolute rates are not.
...
Criteria vary 10 to 38 per prompt, so criterion pass rates weight long-criteria
...
- Does the Implicit-instruction weakness (63 to 68%) reproduce on the remaining 10 Implicit prompts in the set?
