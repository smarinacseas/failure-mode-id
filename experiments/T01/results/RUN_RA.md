...
- **Training health (through step 56):** format_ok 1.0 throughout; KL ~1e-3 (tiny); length
  noisy-flat ~350 tok (no runaway), cap-hit ~0 to 8%; GPU ~28.7 GB (comfortable). No instability.
...
- **checkpoint-50 preserved at pause time** (`results/adapters/T01-RA/checkpoint-50/`:
  ...
  Steps 51 to 56 discarded. ⚠️ **This checkpoint was later rotated out** by `save_total_limit=3`
...
- Live log rebuilt to a continuous 1→150 trajectory (steps 1 to 50 from the original run that produced
  checkpoint-50; discarded 51 to 56 archived to `grpo_RA_coverage_pre-resume_raw.jsonl`).
...
Over the full 1 to 150 trajectory (robust estimators, not single endpoints;
...
Full 1 to 300 trajectory (`results/logs/RA_full_run.json`):
