...
- **Ran in a single pass** (~1h41m, ~11 to 20s/step), precision rollouts are short (mean_len ~150 to 370
  tok vs RA's ~430 to 490), so no phasing/3h-cap issue. `time_budget_hit=false`.
...
Full 1 to 300 trajectory (`results/logs/RB_full_run.json`):
...
reward comparable to RA (~0.72 to 0.75), but with a **weaker/noisier trend** (t=2.79 vs RA's t=20.24).
