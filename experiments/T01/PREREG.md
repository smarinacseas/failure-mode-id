...
requirement: A-prompts (6 to 12 heterogeneous, buried constraints) and B-prompts
(2 to 4 exactness-demanding constraints) must be sortable at a glance, or the
interaction collapses toward 0 by construction. Difficulty calibrated to a
30 to 70% base-model criterion-pass band (above IFEval difficulty).
...
- **Power context (reported, not a gate):** 200 prompts/cause × k=3 ⇒ ~3 to 4-point
  detection floor for the interaction (vs CC-75's 5 to 8 points: why Tier-2 is
  descriptive only).
...
pre-registered GRPO range **5e-6 to 1e-5**; pick the LR with the healthiest reward
...
criterion-pass was **out of the PREREG §5 30 to 70% band**: 86.4% at the GRPO
...
- Hardened count-based generators (harder for the base 3B, still teacher-tractable):
  keyword_include 4 to 6 → **7 to 10** keywords; required_sections 3 to 5 → **6 to 8**;
  keyword_exclude 1 to 2 → **3 to 5**.
- Retired the two near-free binary types from **sampling**: no_placeholders
  (base ~98 to 100% pass) and title (~92%), which held the mean above band. Their
...
- Constraints per coverage prompt: 5 to 6 → **6 to 7** (k capped at |sample pool| = 7).
